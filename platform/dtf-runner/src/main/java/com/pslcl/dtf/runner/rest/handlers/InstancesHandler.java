...
public final class InstancesHandler implements Handler<RoutingContext>
{
    private final Logger logger;
    private final BlockingExecutor executor;
    private final RestServiceStorage storage;

    public InstancesHandler(BlockingExecutor executor, RestServiceStorage storage)
    {
    ...
    public void handle(RoutingContext context)
    {
        if(context.request().method() != HttpMethod.GET)
            VertxUtil.errorResponse(context, HttpStatus.INTERNAL_SERVER_ERROR, ErrorCodes.INTERNAL_SERVER_ERROR, "InstancesHandler called with " + context.request().method());
