...
public class BindHandlerA {
	...
	private List<Future<ResourceReserveDisposition>> currentFuturesOfRRD;
    private List<ReservedResource> currentReservedResources;
    private List<ReservedResource> reservedResources;
//    private Map<ResourceProvider, List<ReservedResource>> mapReservedResources;
	 ...
	 * @param setSteps
	 */
	public BindHandlerA(InstancedTemplate iT, List<String> setSteps, int iBeginSetOffset) throws NumberFormatException {
		...
		this.currentFuturesOfRRD = new ArrayList<>();
    ...
    private boolean currentFuturesOfRRD_done() {
    	for (Future<ResourceReserveDisposition> future : currentFuturesOfRRD) {
    		if (!future.isDone())
    			return false;
    	}
    	return true;
    ...
    /**
     * Proceed as far as possible, then return. Set done only when bind completes or errors out.
     * 
    ...
    void proceed() throws Exception {
    	try {
    		while (true) {
	    		if (this.reserveInProgress) { // when cleared, .reserveInProgress is never set again
	    			// reserve loop
	    			ResourceProvider rp = null;
	    			if (!this.reserveResourceRequests.isEmpty()) {
		    			if (currentFuturesOfRRD.isEmpty()) {
		    				// ask next-in-line ResourceProvider to reserve resources in reserveResourceRequests
...
//		    					log.debug(simpleName + rps.size() + " resource providers reserved " + this.mapReservedResources.size() +
//		    							                            " resources, but cannot reserve " + this.reserveResourceRequests.size());
		    					throw new Exception("resource providers cannot reserve all resources required by template bind steps");
		    				}
		    				rp = this.resourceProviders.getProviders().get(this.nextIndexResourceProvider++);
		    				...
		    				this.currentFuturesOfRRD = resourceProviders.reserve(this.reserveResourceRequests, 1000*60 * 6);
		    			}

//		    			// return if next future not ready
//		    			Future<ResourceReserveDisposition> future = this.currentFuturesOfRRD.get(0);
//		    			if (!future.isDone())
//		    				return;
//		    			// get ResourceReserveDisposition
//		    			ResourceReserveDisposition rrd = future.get();
		    			
		    			// because we plan to use our parallel bind call (n resources bind in parallel), this proceed() method returns, to be called later, until all the reserve futures are done
		    			if (!this.currentFuturesOfRRD_done())
		    				return;
		    			
		    			// all reserve futures ready, process them all: add each embedded ReservedResource to this.reserveResourceRequests
		    			for (Future<ResourceReserveDisposition> future : this.currentFuturesOfRRD) {
		    				ResourceReserveDisposition rrd;
							try {
								rrd = future.get();
								...
								log.debug(simpleName + "proceed() fails in reserve with exception " + e1);
								throw e1;
							}
		    			}
...
//		    			this.mapReservedResources.put(rp, this.currentReservedResources); // list may not be in any order, but each element has its own step reference stored
		    			continue; // process more resource reserve requests
	    			} else {
	    				// move from reserving resources to binding them
	    				this.reserveInProgress = false;
	    				
	    				// logging computation
	    				int count = this.reservedResources.size();
//	    				int count = 0;
//	    				ResourceProvider [] arrayMappedRPs = (ResourceProvider [])this.mapReservedResources.keySet().toArray();
//	    				for (ResourceProvider mappedRP : arrayMappedRPs) {
//	    					List<ReservedResource> listRR = this.mapReservedResources.get(mappedRP);
//	    					count += listRR.size();
//	    				}
	    				log.debug(simpleName + "has reserved " + count + " resources");
	    			}
	    		} // end if(reserveInProgress)

	    		// The parallel bind call. Start multiple asynch binds. Each bind is performed by one specific resource provider, the same one that reserved the resource in the first place. 
